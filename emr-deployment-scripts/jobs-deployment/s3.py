...
def get_latest_object(bucket: str,
                      prefix: str,
                      filter_predicate: Callable[[str], bool] = lambda x: True) -> Optional[Tuple[str, datetime]]:
    s3 = boto3.client('s3')
    ...
    if 'Contents' not in response:
        ...
        sorted_keys_by_ts_desc = [(obj['Key'], obj['LastModified']) for obj in sorted_objs_by_ts_desc if
                                  filter_predicate(obj['Key'])]
